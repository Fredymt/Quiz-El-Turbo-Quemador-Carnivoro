...
const App: React.FC = () => {
  ...
  const handleTargetWeightSelect = (targetWeightData: WeightData) => {
    ...
    if (bmi < 18.5) { bmiCategory = "Bajo peso"; risk = "Riesgo por nutrición insuficiente y metabolismo inestable."; }
    else if (bmi >= 18.5 && bmi < 25) { bmiCategory = "Peso normal"; risk = "Riesgo bajo. Tendencia saludable general."; }
    else if (bmi >= 25 && bmi < 30) { bmiCategory = "Sobrepeso"; risk = "Mayor riesgo de aumento de peso, resistencia a la insulina y baja energía."; }
    else if (bmi >= 30) { bmiCategory = "Obesidad"; risk = "Alto riesgo: alteraciones metabólicas, inflamación y dificultad para quemar grasa."; }
    ...
    const derived: DerivedStats = { heightM, currentWeightKg: currentKg, targetWeightKg: targetKg, weightDifferenceKg: diff, goalDirection: direction, bmi, bmiCategory, risk, idealWeightMin, idealWeightMax, differenceToIdeal };
    setAnswers(prev => ({ ...prev, targetWeight: targetWeightData, derived }));
    setProgress(65);
    setCurrentStep(AppStep.PROCESSING); 
  };

  const handleProcessingComplete = () => { setCurrentStep(AppStep.BMI_RESULT); setProgress(75); };
  const handleBMIResultNext = () => { setCurrentStep(AppStep.IDEAL_WEIGHT); setProgress(85); };
  const handleIdealWeightNext = () => { setCurrentStep(AppStep.DIET_FAMILIARITY); setProgress(90); };
  const handleDietFamiliaritySelect = (familiarity: DietFamiliarity) => { setAnswers(prev => ({ ...prev, dietFamiliarity: familiarity })); setCurrentStep(AppStep.MEAT_PREFERENCE); setProgress(95); };
  const handleMeatPreferenceNext = (selectedMeats: MeatType[]) => { const includeAll = selectedMeats.includes('all'); setAnswers(prev => ({ ...prev, preferredMeats: selectedMeats, includeAllMeats: includeAll })); setCurrentStep(AppStep.FISH_CONSUMPTION); setProgress(98); };
  const handleFishConsumptionSelect = (fishConsumption: FishConsumption) => { setAnswers(prev => ({ ...prev, fishConsumption })); setCurrentStep(AppStep.DIET_EXPLANATION); setProgress(100); };
  const handleDietExplanationNext = () => { setCurrentStep(AppStep.MOTIVATION); setProgress(100); };
  const handleMotivationNext = () => { setCurrentStep(AppStep.WATER_INTAKE); setProgress(100); };
  const handleWaterIntakeSelect = (waterIntake: WaterIntake) => {
    let hydrationLevel: HydrationLevel = 'unknown';
    switch (waterIntake) { case 'coffee_tea': hydrationLevel = 'very_low'; break; case 'less_16oz': hydrationLevel = 'low'; break; case '16_48oz': hydrationLevel = 'moderate'; break; case '56_80oz': hydrationLevel = 'high'; break; case 'no_count': hydrationLevel = 'unknown'; break; }
  ...
  const handleDailyRoutineSelect = (dailyRoutine: DailyRoutine) => {
    ...
    if (dailyRoutine === 'desk_job' || dailyRoutine === 'home') { metabolicActivity = 'sedentary'; } else if (dailyRoutine === 'active_move') { metabolicActivity = 'active'; } else if (dailyRoutine === 'always_exercise') { metabolicActivity = 'very_active'; }
  ...
  const handleSleepSelect = (sleepHours: SleepHours) => {
    ...
    switch (sleepHours) { case 'less_than_5': sleepQuality = 'very_low'; break; case '5_6': sleepQuality = 'low'; break; case '7_8': sleepQuality = 'ideal'; break; case 'more_than_8': sleepQuality = 'good'; break; }
  ...
  const handleWeightSatisfactionSelect = (time: WeightSatisfactionTime) => {
    ...
    switch (time) { case 'less_than_year': emotionalState = 'recent'; break; case 'one_to_three_years': emotionalState = 'moderate'; break; case 'more_than_three_years': emotionalState = 'chronic'; break; case 'never': emotionalState = 'lifelong'; break; }
    setAnswers(prev => ({ ...prev, weightSatisfactionTime: time, emotionalState }));
    setCurrentStep(AppStep.BENEFITS_INFO);
    setProgress(100);
  };
  const handleBenefitsNext = () => { setCurrentStep(AppStep.FAST_RESULTS); setProgress(100); };
  const handleFastResultsNext = () => { setCurrentStep(AppStep.PERSONAL_PLAN); setProgress(100); };
  const handlePersonalPlanNext = (email: string) => {
    setAnswers(prev => ({ ...prev, email }));
    if (window.fbq) { window.fbq('track', 'Lead', { email: email }); }
    ...
    if (bmi < 25) { minLoss = 0.25; maxLoss = 0.5; } else if (bmi < 30) { minLoss = 0.5; maxLoss = 1.0; } else { minLoss = 0.8; maxLoss = 1.5; }
    ...
    let daysTotal = 30; if (kgToLose > 0) { const weeks = kgToLose / estimatedWeeklyLoss; daysTotal = Math.round(weeks * 7); } if (daysTotal < 30) daysTotal = 30;
    ...
    const getFullDateLabel = (d: Date) => { const day = d.getDate(); const month = d.toLocaleString('es-ES', { month: 'long' }); return `${day} de ${month}`; };
    ...
    const allWeights = points.map(p => p.weight), minW = Math.min(...allWeights), maxW = Math.max(...allWeights), margin = (maxW - minW) * 0.2 || 5, scaleMax = maxW + margin, scaleMin = minW - margin, scaleRange = scaleMax - scaleMin;
    points.forEach(p => { p.y = ((scaleMax - p.weight) / scaleRange) * 100; });
    const targetDateLabel = getFullDateLabel(targetDate);
    const finalProjection: FinalProjection = { headlineLine1: "En base a tus respuestas,", headlineLine2: "predecimos que pesarás", headlineLine3: `${targetWeight} kg el ${targetDateLabel}.`, finalWeight: targetWeight, finalDate: formatDateISO(targetDate), points };
    setAnswers(prev => ({ ...prev, derived: { ...prev.derived!, projection: finalProjection } }));
    setCurrentStep(AppStep.FINAL_PROCESSING);
    setProgress(100);
  };

  const handleFinalProcessingComplete = () => { setCurrentStep(AppStep.RESULTS); };
  const handleResultsContinue = () => { if (!answers.derived) return; setCurrentStep(AppStep.FINAL_OFFER); };
  const handleFinalPurchase = () => { alert("¡Gracias por tu compra! Redirigiendo a pasarela de pago..."); };
  const handleBack = () => {
    switch (currentStep) {
